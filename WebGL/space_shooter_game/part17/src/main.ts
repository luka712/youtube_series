...
engine.initialize().then(() => 
{
    ...
    const enemyManager = new EnemyManager(engine.clientBounds[0], 
        ...
        bulletManager,
        highScore);
    const grayscaleEffect = engine.effectsFactory.createGrayScaleEffect();
    const textureEffect = engine.effectsFactory.createTextureEffect();
    textureEffect.setTexture(Content.iceTexture);

    document.getElementById("mix")?.addEventListener("input", (e) => {
        const input = e.target as HTMLInputElement;
        textureEffect.mixValue = parseFloat(input.value);
    });
